...
GLOBAL_LOOP = None
APP_VERSION = "2026.09.15"
GITHUB_REPO_URL = "https://github.com/qianyuniao/qianyuniao-Canvas"
GITHUB_VERSION_URL = "https://raw.githubusercontent.com/qianyuniao/qianyuniao-Canvas/main/VERSION"
GITHUB_TREE_URL = "https://api.github.com/repos/qianyuniao/qianyuniao-Canvas/git/trees/main?recursive=1"
GITHUB_RAW_ROOT = "https://raw.githubusercontent.com/qianyuniao/qianyuniao-Canvas/main"
GITHUB_UPDATE_NOTES_URL = GITHUB_RAW_ROOT + "/static/update-notes.json"
MODELSCOPE_REPO_URL = "https://modelscope.ai/studios/qianyuniao/qianyuniao-Canvas"
MODELSCOPE_RAW_ROOT = "https://www.modelscope.ai/studios/qianyuniao/qianyuniao-Canvas/raw/main"
# ModelScope 仓库默认分支为 master；raw 网页路径会返回 HTML，必须用仓库文件 API 才能拿到纯文本
# 注意：API 路径大小写敏感（推送/文件 API 用大写会 404/拒绝）
MODELSCOPE_FILE_API_ROOT = "https://www.modelscope.ai/api/v1/studio/qianyuniao/qianyuniao-Canvas/repo?Revision=master&FilePath="
...
MODELSCOPE_TREE_URL = "https://www.modelscope.ai/api/v1/studio/qianyuniao/qianyuniao-Canvas/repo/files?Revision=master&Recursive=true"
...
def fetch_remote_update_notes(url: str, version: str = "", timeout: float = 5.0) -> Dict[str, Any]:
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "qianyuniao-Canvas-Updater"},
            timeout=timeout,
...
STATIC_PROMPT_TEMPLATE_MD = os.path.join(STATIC_DIR, "system-prompts", "qianyuniao-canvas-prompt-templates.md")
...
def github_json(url: str, use_etag_cache: bool = False):
    headers = {
        ...
        "User-Agent": "qianyuniao-Canvas-Updater",
    }
    cache_key = url
...
def github_bytes(url: str) -> bytes:
    resp = github_get(url, headers={"User-Agent": "qianyuniao-Canvas-Updater"}, timeout=60)
    return resp.content
...
def modelscope_update_file_list() -> List[str]:
    ...
    resp = github_get(MODELSCOPE_TREE_URL, headers={"User-Agent": "qianyuniao-Canvas-Updater"}, timeout=30)
    payload = json.loads(resp.content.decode("utf-8", errors="replace"))
...
async def import_local_assets_from_urls(payload: LocalAssetUrlImportRequest):
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, headers={"User-Agent": "qianyuniao-Canvas-Asset-Importer/1.0"}) as client:
...
async def submit_midjourney(payload: MidjourneySubmitRequest):
    ...
    if mode == "blend":
        ...
        body = {"image_urls": image_urls, "size": size, "speed": speed, "metadata": {"source": "qianyuniao-canvas"}}
        ...
        body = {"prompt": prompt, "image_urls": image_urls, "size": size, "speed": speed, "metadata": {"source": "qianyuniao-canvas"}}
        path = "/v1/midjourney/generations/edits"
    else:
        if not prompt:
            raise HTTPException(status_code=400, detail="Midjourney 文生图需要提示词。")
        body = {"prompt": prompt, "size": size, "version": str(payload.version or "6.1").strip()[:24], "speed": speed, "metadata": {"source": "qianyuniao-canvas"}}
        if image_urls:
...
async def submit_midjourney_action(payload: MidjourneyActionRequest):
    ...
    if speed not in MIDJOURNEY_SPEEDS:
        raise HTTPException(status_code=400, detail="Midjourney 速度仅支持 relax、fast 或 turbo。")
    body = {"task_id": task_id, "speed": speed, "metadata": {"source": "qianyuniao-canvas"}}
...
async def submit_midjourney_modal(payload: MidjourneyModalRequest):
    ...
    body = {
        ...
        "mask_url": mask_url,
        "speed": speed,
        "metadata": {"source": "qianyuniao-canvas"},
    }
...
def canvas_workflow_payload(nodes, connections, resources=None):
    return {
        "format": "qianyuniao-canvas-workflow",
        "version": 1,
        "exported_at": now_ms(),
        "nodes": nodes or [],

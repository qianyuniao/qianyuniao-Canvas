(function(){
    ...
    function autoScale(){
        ...
        // 基准视口从 1500x940 放宽到 1420x920，缩放下限从 0.68 提到 0.80。
        // 原参数下 1280x720 这类常见窗口会把整个界面压到 68%，
        // 字号被二次缩小，是"文字看不清"的主因。
        // 溢出风险仍由下方 scheduleContentFit() 兜底（内容超宽时会自动再降）。
        const compactRatio = Math.min(viewportWidth / 1420, viewportHeight / 920);
        if(compactRatio < 1) {
            return Math.max(0.80, Math.min(1, compactRatio));
        }
